...
mutable struct Parameters
    ...
    [7639, 7082, 6524, 5674, 4725, 4147, 3928, 3362,
              2636, 1970, 1468, 1166, 943, 718, 455, 244],5,1/24, 0)
...
function kato_katz(d, gamma_k)
    gamma1 = rand(gamma_k)[1]
    gamma2 = rand(gamma_k)[1]
    pois1 = rand(Poisson(1*gamma1*d.eggs))[1]
    pois2 = rand(Poisson(1*gamma2*d.eggs))[1]
...
function save_population_to_file(filename, humans, miracidia, cercariae)
    save(filename, "humans", humans,  "miracidia", miracidia, "cercariae", cercariae)
end
...
function update_env_to_equilibrium(num_time_steps, humans, miracidia, cercariae, pars)
    ...
    for j in 1:num_time_steps
        ...
        if sim_time >= record_time
            a = get_prevalences!(humans, sim_time)
            push!(record, a)
            record_time += pars.record_frequency
        end
...
function run_repeated_sims_no_population_change(filename, num_time_steps, pars, mda_info, vaccine_info)
    ...
    for run in 1:num_repeats
        ...
        humans,  miracidia, cercariae = load_population_from_file(filename)
...
function update_env(num_time_steps, humans, time_step, average_worm_lifespan,
    max_fecundity, r, worm_stages, predis_aggregation,vaccine_effectiveness,
    density_dependent_fecundity, miracidia,env_cercariae,
    contact_rate, env_cercariae_death_rate, env_miracidia_death_rate,
    female_factor, male_factor, contact_rates_by_age, death_rate_per_time_step,
    birth_rate, mda_info, vaccine_info, mda_adherence, record_frequency)
    ...
    for j in 1:num_time_steps
        ...
        if sim_time >= record_time
            a = get_prevalences!(humans, gamma_k, sim_time)
            push!(record, a)
            record_time += record_frequency
        end
        ...
        if l > 0
            for i in 1:l
                humans = birth_of_human(humans, female_factor, male_factor,
                    contact_rates_by_age, death_rate_per_time_step, worm_stages, predis_aggregation)
            end
        end
    end
    return humans, record
end
